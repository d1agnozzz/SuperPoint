...
class VGGBlock(torch.nn.Sequential):
    def __init__(self, c_in, c_out, kernel_size, relu=True):
        padding = (kernel_size - 1) // 2
        conv = torch.nn.Conv2d(
            c_in, c_out, kernel_size=kernel_size, stride=1, padding=padding
        )
        activation = torch.nn.ReLU(inplace=True) if relu else torch.nn.Identity()
        bn = torch.nn.BatchNorm2d(c_out, eps=0.001)
        super().__init__(
            OrderedDict(
                [
                    ("conv", conv),
...
class SuperPointUnofficial(torch.nn.Module):
    default_conf = {
        "descriptor_dim": 256,
        "nms_radius": 4,
    ...
    def __init__(self, **conf):
        ...
        for i, c in enumerate(channels[1:], 1):
            ...
            backbone.append(torch.nn.Sequential(*layers))
    ...
    def forward(self, data):
        ...
        return {
            "keypoints": keypoints[0].T.detach().numpy(),
            "keypoint_scores": scores[0].T.detach().numpy(),
            "descriptors": descriptors[0].T.detach().numpy(),
        }
...
class SuperPointFrontend(object):
    ...
    def __init__(
        ...
        self.net = SuperPointNet() if use_official else SuperPointUnofficial()
        if cuda:
    ...
    def run(self, img):
        ...
        if not self.use_official:
            return outs["keypoints"], outs["descriptors"], outs["keypoint_scores"]
...
class PointTracker(object):
    ...
    def update(self, pts, desc):
        ...
        if pts is None or desc is None:
            ...
            return
        print(f'desc type: {type(desc)}')
        print(f'desc.shape[1]: {desc.shape[1]}')
        print(f'pts type: {type(pts)}')
        print(f'pts.shape[1]: {pts.shape[1]}')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Specify using official MagicLeap model",
    )
    ...
    fe = SuperPointFrontend(
        ...
        cuda=opt.cuda,
        use_official=not opt.use_unofficial,
    )
